...
*Download failed (missing/invalid TS segments); the ts directory is kept for resume.*
...
*No valid TS files in the download dir — check the URL.*
...
## "[Error] 解析 m3u8 地址失败"
*m3u8 URL parse failed.* The argument is not an `http(s)` URL, or the playlist
is malformed/gzip-corrupt. Quote the URL, re-check it.

## "缺少 m3u8 地址..."
*Missing m3u8 URL.* Pass `-u <url>` or `--list <file>`.

## "读取列表失败" / "列表为空"
*List file unreadable / empty* (batch mode). Check the file path and that it
